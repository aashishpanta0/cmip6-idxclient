...
def _build_var_name(variable, model = None, scenario = None, quality = None,rawtime=None,l=None,u=None):
    ...
    if rawtime:
        var_name = var_name + f',t:{rawtime}'
    if l:
        lbb=create_bb(l)
        var_name = var_name + f',l:{lbb}'
    if u:
        ubb=create_bb(u)
...
class DXInterface:
    ...
    def calculate_day_of_year(self,date_str):
    ...
    def format_date(self,date_str):
        date = datetime.strptime(date_str, '%Y-%m-%d')
        day_of_year = self.calculate_day_of_year(date_str)
    ...
    def _query_pc(self, variable, start_date, end_date, model = None, scenario = None, quality = None, lb=None, ub=None,geo_lb = (-60.0,-180.0), geo_ub = (90.0,180.0)):
        ...
        lat = [(l * .25) - 59.875 for l in range(glb[0], gub[0]+1)]
        lon = [(l * .25) - 179.875 for l in range(glb[1], gub[1]+1)]
        return(data, lat, lon)
